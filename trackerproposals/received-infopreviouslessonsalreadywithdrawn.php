...
$query="SELECT * FROM tracker WHERE id_user_teacher = '$mi_identificador' AND paid=1 AND releasefunds=1 AND moneywithdrawnbyteacher=1 ORDER BY start_time_unix ASC ";
...
$nuevos=mysqli_num_rows($result);
 ...
 if (!$nuevos)
        echo " No money withdrawn so far.";
...
for($i=0;$i<$nuevos;$i++)
{
	...
	$session_releasefunds_timestamp=$fila['timestamp_releasefunds'];
	$session_moneywithdrawn_timestamp=$fila['timestamp_moneywithdrawnbyteacher'];
	...
	echo "<li>Info Session payment released timestamp: $session_releasefunds_timestamp</li>";
	echo "<li>Info Session payment withdrawn timestamp: $session_moneywithdrawn_timestamp</li>";
	echo "</ul>";
