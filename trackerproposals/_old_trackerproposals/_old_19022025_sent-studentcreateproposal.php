...
$(document).ready(function() {
  ...
  $("input[name='submit1']").on("click", function(event) {
    ...
    if (!startDate || !startHour || !startMinute || !startAMPM || !duration || !description || !language) {
      alert("Por favor, rellena todos los campos antes de continuar.");
      return;
    }

    // Crear el contenido de la previsualización (en formato de 12 horas)
    var previewContent = `
      <li><strong>Idioma a aprender:</strong> ${language}</li>
      <li><strong>Fecha de inicio:</strong> ${startDate}</li>
      <li><strong>Hora de inicio:</strong> ${startHour}:${startMinute} ${startAMPM}</li>
    ...
    $("#preview-container").show();// Mostrar el contenedor de la previsualización    
    $("#class-form").hide(); // Ocultar solo el formulario 
    $(".post-comment-box").show();// Asegurarnos de que los contenedores que no oculatamos en visibles
    $("input[name='submit1']").hide();// Ocultamos el botón "See proposed classes"
  });
