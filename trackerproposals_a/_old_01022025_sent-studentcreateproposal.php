...
if (isset($_POST['start_date'], $_POST['start_time'], $_POST['duration'], $language_taught )  )
{
	...
	{	
		...
		INSERT INTO tracker (	created_from_recurrent,id_user_teacher,id_user_student,time_shift_student,start_time_unix,end_time_unix,date_start_local,date_end_local,session_lenght_minutes,
								language_taught,hourly_rate_original, price_session_total, description_session, created_timestamp )
		VALUES(					'$is_recurrent','$teacher_id_orden','$my_id_orden','$my_timeshift','$unixtime_start_insert_db','$unixtime_end_insert_db','$date_start_insert_db','$date_end_insert_db','$duration',
								'$language_taught','$hourly_rate','$total_session_price','$description',NOW())
...
<br><br>
  ...
  <br>
	...
	<br>
	<br>
	Select language to be taught and your level (if you want to learn a language that is not in your list you need to add it -link to the addlanguage.php-)<br><br>
	
	<select id="language_to_learn" name="language_to_learn">
 
	<?php
	list($learn_langs_array_multidim["$my_id_orden"], $learn_langs_full_name_array_multidim["$my_id_orden"], 
							$learn_langs_level_array_multidim["$my_id_orden"], 
							$learn_langs_forshare_array_multidim["$my_id_orden"], 	$learn_langs_price_array_multidim["$my_id_orden"], 
							$learn_langs_typeofexchange_array_multidim["$my_id_orden"], 
							$learn_langs_priceorexchangetext_array_multidim["$my_id_orden"], $learn_langs_level_image_array_multidim["$my_id_orden"], 
							$learn_langs_2letters_array_multidim["$my_id_orden"])
							= lenguas_que_quiere_estudiar_usuario($my_id_orden,$link);
							
							
							
	for($uu=0;$uu<count($learn_langs_array_multidim["$my_id_orden"]);$uu++ )
	{
		?> <option value="<?php echo $learn_langs_array_multidim["$my_id_orden"][$uu]; ?>"><?php echo $learn_langs_full_name_array_multidim["$my_id_orden"][$uu];; ?></option><?php
		
	}
							
	print_r($learn_langs_full_name_array_multidim["$my_id_orden"]);					
  ...
  <br>
	...
	if( ($latitud1==0 AND $longitud1==0)  OR  ($latitud2==0 AND $longitud2==0) )
	{
		...
		echo "Tipo de reunion: ----$type_of_meeting-----";
	}
