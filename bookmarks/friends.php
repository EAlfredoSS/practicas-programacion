                        ...
                        <div class="posts-section">
                            ...
                            <div id="seccion_teach" class="post-bar">
                                ...
                                <div class="container-fluid m-0 p-0" id="user-cards-container">
                                    ...
                                    if (isset($_GET['action_block']) && isset($_GET['friend_id'])) {
                                        $action_block = $_GET['action_block'];
                                        $friend_id = intval($_GET['friend_id']);
                                        
                                        if ($action_block == 'block') {
                                            $check_block = mysqli_query($link, "SELECT * FROM blocked_users WHERE blocker = $identificador2017 AND blocked = $friend_id");
                                            if (mysqli_num_rows($check_block) == 0) {
                                                mysqli_query($link, "INSERT INTO blocked_users (blocker, blocked, created_at) VALUES ($identificador2017, $friend_id, NOW())");
                                            }
                                        } else if ($action_block == 'unblock') {
                                            mysqli_query($link, "DELETE FROM blocked_users WHERE blocker = $identificador2017 AND blocked = $friend_id");
                                        }
                                    ...
                                    if ($stmt = $link->prepare($query)) {
                                        $stmt->bind_param("ss", $identificador2017, $identificador2017);
                                        $stmt->execute();
                                        ...
                                        if ($result->num_rows > 0) {
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                $query_i_blocked = "SELECT * FROM blocked_users WHERE blocker = $identificador2017 AND blocked = $bookmarked_user_id";
                                                $result_i_blocked = mysqli_query($link, $query_i_blocked);
                                                $is_blocked = (mysqli_num_rows($result_i_blocked) > 0);
