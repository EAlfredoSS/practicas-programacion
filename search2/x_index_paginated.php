...
if ($form_submitted || $highlight_user > 0) {
    ...
    // - Checkbox ON: solo usuarios con precio válido dentro del rango; EXCLUYE sin precio.
    // - Checkbox OFF: solo usuarios SIN precio para el idioma seleccionado.
    if (
        ...
        // Handle "30+" case: if 0 (empty) or >= 30, treat as "No Upper Limit"
        ...
        // Checkbox OFF: mostrar SOLO usuarios SIN precio para el idioma seleccionado
        $esc_learns = mysqli_real_escape_string($link, $userislearning);
        $where_clause .= " AND EXISTS (
            SELECT 1 FROM my_langs ml
            JOIN languages1 lml ON lml.Id = ml.lang_id
    ...
    // Filtro para Students: candidato quiere aprender lo que YO enseño Y enseña lo que YO quiero aprender
    if (isset($_GET['partner']) && $_GET['partner'] === 'student' && !empty($userislearning) && !empty($useristeaching)) {
        ...
        $level_condition_teaches = $use_teaches_level_filter
            ? " AND ll.level_id BETWEEN $teaches_min_level AND $teaches_max_level" : "";
        ...
        $level_condition_learns = $use_learns_level_filter
            ? " AND ml.level_id BETWEEN $learns_min_level AND $learns_max_level" : "";
...
if (!$result || !mysqli_num_rows($result)) {
    ...
    while ($fila = mysqli_fetch_array($result)) {
        ...
        array_push($ciudad_usuarios,  $fila['Ciudad']);
...
// ============================================================
// OBTENER LISTA DE SUBIDIOMAS (códigos I_Id)
// ============================================================
$subidiomas_list = [];
$sub_res = mysqli_query($link, "SELECT DISTINCT I_Id FROM languages_macrolanguages");
if ($sub_res) {
    while ($row_sub = mysqli_fetch_assoc($sub_res)) {
        $subidiomas_list[] = strtolower($row_sub['I_Id']);
    }
    mysqli_free_result($sub_res);
}

// =====================================================================
// getFlagPath — ahora acepta un tercer parámetro para forzar placeholder en subidiomas
// =====================================================================
function getFlagPath($code2, $code3, $is_sublang = false)
{
    // Si es subidioma, devolvemos placeholder directamente
    if ($is_sublang) {
        $placeholders = [
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .sublang-indicator {
            margin-left: 2px;
            /* más pegado al código */
            font-size: 9px;
            /* antes 8px → también más grande */
            font-weight: 400;
            color: #888;
...
<body>
    ...
    for ($i = 0; $i < $number_of_affected_users; $i++) {
        if (isset($orden_usuarios[$i])) {
            ...
            $teaches_langs = array_map(function ($l) {
                return (strtolower($l) === 'val' || strtolower($l) === 'valencian') ? 'cat' : $l;
            }, $teaches_langs);
            $learns_langs  = array_map(function ($l) {
                return (strtolower($l) === 'val' || strtolower($l) === 'valencian') ? 'cat' : $l;
            }, $learns_langs);
                    ...
                    ?>
                            ...
                            <div class="user-card" id="user-<?php echo $orden_usu; ?>" data-user-id="<?php echo $orden_usu; ?>">
                                    ...
                                    <div class="card-identity">
                                        <div class="card-name-row">
                                            ...
                                            <?php if ($rating_display !== null): ?>
                                                <span class="rating-text"><?php echo $rating_display; ?></span>
                                            <?php endif; ?>
                                        </div>
                                ...
                                <div class="card-langs">
                                        ...
                                        <div class="lang-grid">
                                            ...
                                            if (!empty($my_langs_array_multidim[$orden_usu])):
                                                foreach ($my_langs_array_multidim[$orden_usu] as $sss => $lang_code_raw):
                                                    ...
                                                    $is_sublang  = in_array(strtolower($lang_code_raw), $subidiomas_list);
                                                    $flag_path   = getFlagPath($codig_2, $lang_code_raw, $is_sublang);
                                            ...
                                            ?>
                                                    <div class="flag-wrapper">
                                                        ...
                                                        <span class="lang-code">
                                                            <?php echo strtolower($lang_code_raw) ?: '&nbsp;'; ?>
                                                            <?php if ($is_sublang): ?>
                                                                <span class="sublang-indicator">sub</span>
                                                            <?php endif; ?>
                                                        </span>
                                    ...
                                    <div class="lang-section" style="margin-bottom: 8px;">
                                        <span class="lang-label">
                                ...
                                <div class="card-footer">
                                    <span class="card-footer-dist"><?php echo htmlspecialchars("$ciudad area" ?: 'Unknown area'); ?></span>
                                    <?php if ($show_org_badge): ?>
