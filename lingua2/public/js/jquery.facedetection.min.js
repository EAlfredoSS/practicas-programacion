! function (n) {
    ...
    try {
        var y = function () {
                ...
                for (var n = document.getElementsByTagName("script"), p = 0; p < n.length; p++)
                    if (n[p].src.match(/(jquery\.facedetection(\.min)?\.js)|\/ccv\.js/)) return n[p].src
            },
            x = y()
    } catch (n) {}
    if (void 0 === e) {
            ...
            return e.core[p.toString()] = p().core,
                function () {
                    ...
                    if (arguments.length > 1)
                        for (y = arguments[arguments.length - 2], x = arguments[arguments.length - 1], e = new Array(arguments.length - 2), z = 0; z < arguments.length - 2; z++) e[z] = arguments[z];
                    ...
                    return y ? function (y, t) {
                        ...
                        [s.core.apply(i, [s.pre.apply(i, [1])[0], 0, 1])]
                    ])
                }
        };
        e.core = {}
    ...
    var i = {
            ...
            grayscale: function (n) {
                for (var p, z, y = n.getContext("2d"), x = y.getImageData(0, 0, n.width, n.height), e = x.data, i = n.width * n.height * 4; i > 0;) e[i -= 4] = e[p = i + 1] = e[z = i + 2] = .3 * e[i] + .59 * e[p] + .11 * e[z];
                return y.putImageData(x, 0, 0), n
            },
            array_group: function (n, p) {
                var z, y, x = new Array(n.length);
                for (z = 0; z < n.length; z++) x[z] = {
                    parent: -1,
                    element: n[z],
                    rank: 0
                };
            ...
            detect_objects: e(x, function (n, z, y, x) {
                function e(n) {
                    ...
                    i[0] = p, s[0] = {
                        width: i[0].width,
                        height: i[0].height,
                        data: i[0].getContext("2d").getImageData(0, 0, i[0].width, i[0].height).data
                    };
                    var t;
                    for (t = 1; t <= z; t++) i[4 * t] = document.createElement("canvas"), i[4 * t].width = Math.floor(i[0].width / Math.pow(y, t)), i[4 * t].height = Math.floor(i[0].height / Math.pow(y, t)), i[4 * t].getContext("2d").drawImage(i[0], 0, 0, i[0].width, i[0].height, 0, 0, i[4 * t].width, i[4 * t].height), s[4 * t] = {
                        width: i[4 * t].width,
                        height: i[4 * t].height,
                        data: i[4 * t].getContext("2d").getImageData(0, 0, i[4 * t].width, i[4 * t].height).data
                    };
                    for (t = x; t < e + 2 * x; t++) i[4 * t] = document.createElement("canvas"), i[4 * t].width = Math.floor(i[4 * t - 4 * x].width / 2), i[4 * t].height = Math.floor(i[4 * t - 4 * x].height / 2), i[4 * t].getContext("2d").drawImage(i[4 * t - 4 * x], 0, 0, i[4 * t - 4 * x].width, i[4 * t - 4 * x].height, 0, 0, i[4 * t].width, i[4 * t].height), s[4 * t] = {
                        width: i[4 * t].width,
                        height: i[4 * t].height,
                        data: i[4 * t].getContext("2d").getImageData(0, 0, i[4 * t].width, i[4 * t].height).data
                    };
                    for (t = 2 * x; t < e + 2 * x; t++) i[4 * t + 1] = document.createElement("canvas"), i[4 * t + 1].width = Math.floor(i[4 * t - 4 * x].width / 2), i[4 * t + 1].height = Math.floor(i[4 * t - 4 * x].height / 2), i[4 * t + 1].getContext("2d").drawImage(i[4 * t - 4 * x], 1, 0, i[4 * t - 4 * x].width - 1, i[4 * t - 4 * x].height, 0, 0, i[4 * t + 1].width - 2, i[4 * t + 1].height), s[4 * t + 1] = {
                        width: i[4 * t + 1].width,
                        height: i[4 * t + 1].height,
                        data: i[4 * t + 1].getContext("2d").getImageData(0, 0, i[4 * t + 1].width, i[4 * t + 1].height).data
                        ...
                        data: i[4 * t + 2].getContext("2d").getImageData(0, 0, i[4 * t + 2].width, i[4 * t + 2].height).data
                        ...
                        data: i[4 * t + 3].getContext("2d").getImageData(0, 0, i[4 * t + 3].width, i[4 * t + 3].height).data
                    };
                    return [s]
                }

                function s(n, p, z) {
                    var y, x, e, i, s, t, a = this.shared.cascade,
                        ...
                        for (x = 0; x < a.stage_classifier.length; x++) {
                            ...
                            for (e = 0; e < a.stage_classifier[x].count; e++)
                                for (b[e] = {
                                    ...
                                    }, t = 0; t < _[e].size; t++) b[e].px[t] = 4 * _[e].px[t] + _[e].py[t] * v[_[e].pz[t]], b[e].pz[t] = _[e].pz[t], b[e].nx[t] = 4 * _[e].nx[t] + _[e].ny[t] * v[_[e].nz[t]], b[e].nz[t] = _[e].nz[t]
                        }
                        for (t = 0; t < 4; t++) {
                                    ...
                                    for (x = 0; x < a.stage_classifier.length; x++) {
                                        ...
                                        for (e = 0; e < a.stage_classifier[x].count; e++) {
                                            ...
                                            if (E <= S) C += I[2 * e];
                                                ...
                                                var W, H = !0;
                                                for (W = 0; W < N.size; W++) {
                                                    if (N.pz[W] >= 0 && (D = M[N.pz[W]][k[N.pz[W]] + N.px[W]], D < E)) {
                                                        if (D <= S) {
                                                            H = !1;
                                                            break
                                                        }
                                                    ...
                                                    if (N.nz[W] >= 0 && (j = M[N.nz[W]][k[N.nz[W]] + N.nx[W]], j > S)) {
                                                        if (E <= j) {
                                                            H = !1;
                                                            break
                                                        }
                ...
                function t(n) {
                    var p, z, y = this.shared.min_neighbors,
    ...
    onmessage = function (n) {
        var p;
        try {
            if (p = "string" == typeof n.data ? JSON.parse(n.data) : n.data, "jquery.facedetection" === p.type) {
                var z = {
                        shared: p.shared
                    },
        ...
        return n.isFunction(p) ? x.complete = p : n.extend(x, p), this.each(function () {
            function p() {
                var n, p;
            ...
            function e(n) {
                for (var p = n.length, z = [], e = 0; e < p; ++e) null !== x.confidence && n[e].confidence <= x.confidence || (n[e].positionX = a.left + n[e].x, n[e].positionY = a.top + n[e].y, n[e].offsetX = t.left + n[e].x, n[e].offsetY = t.top + n[e].y, n[e].scaleX = r, n[e].scaleY = h, z.push(n[e]));
