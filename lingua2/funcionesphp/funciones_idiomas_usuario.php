...
function lenguas_que_quiere_estudiar_usuario($usuario_input,$link)
{
	...
	for ($jjj = 0; $jjj < $num_learn_langs; $jjj++) 
	{
		...
		array_push($learn_langs_forshare_array,$fila_learn_langs['for_share']?? null); // DABA UN WARNING Y HEMOS PUESTO '?? null'
		array_push($learn_langs_price_array,$fila_learn_langs['lang_price']?? null);   // DABA UN WARNING Y HEMOS PUESTO '?? null'
...
//'YYYY-MM-DD HH:MM:SS'
function obtenerTiempoUnix($fechaHoraUTC0, $zonaHoraria) {
    try {
        // Crear un objeto DateTime con la fecha y hora en UTC
        $fechaHora = new DateTime($fechaHoraUTC0, new DateTimeZone($zonaHoraria));
        
        // Obtener el tiempo Unix (segundos desde la época Unix)
        return $fechaHora->getTimestamp();
    } catch (Exception $e) {
...
function diferenciaHoraria($fecha, $husoHorario1, $husoHorario2) {
    // Crear objetos DateTime para la fecha en ambos husos horarios
    $dateTime1 = new DateTime($fecha, new DateTimeZone($husoHorario1));
    $dateTime2 = new DateTime($fecha, new DateTimeZone($husoHorario2));

    // Calcular la diferencia en segundos
    $diferencia = $dateTime1->getOffset() - $dateTime2->getOffset();

    // Convertir la diferencia a horas
    $diferenciaSeconds = $diferencia;

    return $diferenciaSeconds;
}
...
*/
 ...
 /*$array_prueba = lenguas_que_conoce_usuario(4588,$link);
