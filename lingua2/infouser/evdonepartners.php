...
$nombre_corto_usu=$fila_46['nombre'];
$nombre_corto_usu_array=explode(' ', $nombre_corto_usu);
$nombre_corto_usu=$nombre_corto_usu_array[0];
...
for ($i = 0; $i < $nuevos_couples1; $i++)
{
    ...
    $ident_del_companero1 = $fila_couples1['user_id_2'];
	
	
	//php8: este if lo pongo porque no se puede meter un null en un array
	if( !is_null($identdeloscompaneros) AND !is_null($ident_del_companero1) )
	{
		array_push($identdeloscompaneros, $ident_del_companero1);
	}
...
for ($i = 0; $i < $nuevos_couples2; $i++) {
    ...
    $ident_del_companero2 = $fila_couples2['user_id_1'];
	
	//php8: este if lo pongo porque no se puede meter un null en un array
	if( !is_null($identdeloscompaneros) AND !is_null($ident_del_companero2) )
	{
		array_push($identdeloscompaneros, $ident_del_companero2);
	}
}
...
//print_r($emaildeloscompaneros);
	...
	//php8: este if lo pongo porque no se puede meter un null en un array
	if( !is_null($identdeloscompaneros) )
	{
		$sizeof_array = count($identdeloscompaneros);
	}

//echo "</br></br>count $sizeof_array ---</br></br>";

for ($i = 0; $i < $sizeof_array; $i++) {
    $ident_de_partner = array_pop($identdeloscompaneros);

    //echo "$i $email_de_partner ";

    $query_detectar_borrados = "SELECT * FROM mentor2009 WHERE orden='$ident_de_partner'";
    ...
    if ($nuevos_detectar_borrados == 0) {
        $query_borrar1 = "DELETE FROM couples2009antiguos WHERE user_id_1 = '$ident_de_partner' AND user_id_2='$identificador2017' ";
        $result_borrar1 = mysqli_query($link, $query_borrar1);
        $nuevos_borrar1 = mysqli_num_rows($result_borrar1);

        $query_borrar2 = "DELETE FROM couples2009antiguos WHERE user_id_2 = '$ident_de_partner' AND user_id_1='$identificador2017' ";
...
<div class="main-section">
 ...
 <div	class="main-ws-sec" >
                            ...
                            <div class="top-profiles ">
                                <div class="pf-hd">
                                    <h3>All <?php echo"$nombre_corto_usu"; ?>'s evaluations

                                        <p style="font-size: 16px"><a href=""><?php echo "$n_comentarios"; ?> evaluations received </a>
