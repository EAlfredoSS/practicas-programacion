...
for ($i = 0; $i < $nuevos_couples1; $i++) {
    ...
    $iden_del_companero1 = $fila_couples1['user_id_2'];
    array_push($idendeloscompaneros, $iden_del_companero1);
...
for ($i = 0; $i < $nuevos_couples2; $i++) {
    ...
    $iden_del_companero2 = $fila_couples2['user_id_1'];
    array_push($idendeloscompaneros, $iden_del_companero2);
}

//print_r($emaildeloscompaneros);

$sizeof_array = count($idendeloscompaneros);

//echo "</br></br>count $sizeof_array ---</br></br>";

for ($i = 0; $i < $sizeof_array; $i++) {
    $iden_de_partner = array_pop($idendeloscompaneros);

    //echo "$i $email_de_partner ";

    $query_detectar_borrados = "SELECT * FROM mentor2009 WHERE orden='$iden_de_partner'";
    ...
    if ($nuevos_detectar_borrados == 0) {
        $query_borrar1 = "DELETE FROM couples2009antiguos WHERE user_id_1 = '$iden_de_partner' AND user_id_2='$identificador2017' ";
        $result_borrar1 = mysqli_query($link, $query_borrar1);
        $nuevos_borrar1 = mysqli_num_rows($result_borrar1);

        $query_borrar2 = "DELETE FROM couples2009antiguos WHERE user_id_2 = '$iden_de_partner' AND user_id_1='$identificador2017' ";
