...
<form id='frame'>
	...
	<fieldset id='language-partner'>
		...
		<div>
			<label class="container">Professional teachers
				<input type='radio' name='partner' id='radio-teachers' checked='checked' />
				<span class="checkmark"></span>
			</label>
			<label class="container">Students
				<input type='radio' name='partner' id='radio-students' value='student' />
				<span class="checkmark"></span>
			</label>
		</div>
	</fieldset>
	...
	<fieldset id="teacher-level" class='student-dialog'>
		<legend>My teacher's level:</legend>
		<select name='teaches'>
			<option value=''>(Select one)</option>
			<?php listIterator($idiomas_equiv, 'asOption'); ?>
		</select>
	...
	<fieldset id='lang-teach' class='student-dialog'>
		...
		<label>My level
			...
			selectSlider('slider2', 'level', $lista_niveles, 0, -2);
			?>
		</label>
	</fieldset>
	<fieldset>
		...
		<label class="container">Even if we cannot understand each other
			<input type="checkbox" name='lang-compatibility'>
			<span class="checkmark"></span>
		</label>
	</fieldset>
		...
		<script type="text/javascript">
			...
			function handleRadioChange() {
				if (radioTeachers.checked) {
					langTeach.style.display = 'none'; // Ocultar fieldset de teachers
					langLearn.style.display = 'block'; // Mostrar fieldset de students
					teacherLevel.style.display = 'none'; //Ocultar fieldet "My teacher's level"
	...
	<fieldset>
		<label class='container'>Global Search:
			<input type='checkbox' name='zone' id="global-search-checkbox" />
			<span class="checkmark"></span>
		</label>
		<div id="osm-map"></div>
	</fieldset>
	<script type='text/javascript'>
		...
		let checkbox = document.getElementById('global-search-checkbox');
		...
		L.circle([<?php echo "$latitud1"; ?>, <?php echo "$longitud1"; ?>], 30000, {
			color: '#e65f00',
			opacity: 1,
			fillColor: '#e65f00',
			fillOpacity: .15
		...
		?>
				L.marker([<?php echo "$latitud_mapa"; ?>, <?php echo "$longitud_mapa"; ?>], {
					icon: studentIcon
				}).addTo(map);
			<?php
			//} else {
	...
	<fieldset>
        ...
        foreach ($distances as $distance) {
            $selected = ($distance == $default) ? 'selected="selected"' : '';
