...
$array_nota_evalu=array();
	...
	for($iiii=0; $iiii < $number_of_affected_users ; $iiii++     )
	{
		...
		WHERE my_l.id='$orden_usu_actual' 
		...
		for ($jjj = 0; $jjj < $num_my_langs; $jjj++) 
		{
			...
			print_r($fila_my_langs);
			
			array_push($my_langs_full_name_array,$fila_my_langs['full_lang_name']);
			//array_push($my_langs_array,$fila_my_langs['lang_id']);
			
			$my_langs_array[$iiii][$jjj]=$fila_my_langs['lang_id'];
			$my_langs_aux=$my_langs_array[$iiii][$jjj];
			
			echo "-- $iiii -- $jjj -- $my_langs_aux <br>";
...
<div class='marco-fichas'>
	...
	<div class='ficha'>
		...
		//print_r($my_langs_array[$i]);
		
		for($kkkk=0;$kkkk<count($my_langs_array[$i]);$kkkk++)
		{
			
			$lang_aux=$my_langs_array[$i][$kkkk];
			echo "$lang_aux ";  
		...
		<div> 
			<?php if( !empty( $my_langs_2letters_array[$kkkk] ) ){ ?>
				<img width="25px" height="16px" src="<?php echo "./banderasseparadas2024/$my_langs_2letters_array[$kkkk].png  "; ?>"  /> 
			<?php } 
